...
class util
{
    ...
    public static function select(UnitEnum $enum): string
    {
        ...
        foreach ($enum::cases() as $case) {
            // For BackedEnum, use the backing value; for regular enums, use the name
            $value = $case instanceof BackedEnum ? $case->value : $case->name;
            $displayText = $case instanceof BackedEnum ? $case->value : $case->name;
            $html .= "<option value='" . htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . "'>" . htmlspecialchars($displayText, ENT_QUOTES, 'UTF-8') . "</option>";
        }
        $html .= "</select>";
        return $html;
    }
